...
//    $jppr.datepick._tdDates = {};
	...
	this._defaults = { // Global defaults for all the date picker instances
        ...
        useFlicker: false, // True to apply ThemeRoller styling, false for default styling
        turnoverBefore: 0,
        turnoverAfter: 0,
...
$jppr.extend(Datepick.prototype, {
	...
	_mainDivClass: ['', 'ui-datepicker ' +
		''], // Popup class --removed ui-widget ui-widget-content ui-helper-clearfix ui-corner-all
	...
	_doMouseOver: function(td, id, timestamp) {
        ...
        if(tdStart.hasClass('availabledate')){
            if(inst.dates.length == 0 || inst.dates.length == 2) {
                monthNotVisible = false;
                for (iCounter = 0; iCounter < turnoverBeforeValue; iCounter++) {
                    tdPrev = tdStart.prev();

                    while (true) {
                        if (tdPrev.hasClass('ui-datepicker-other-month') || tdPrev.hasClass('datepick-other-month')) {
                            tdStart = tdStart.closest('.datepick-one-month, .ui-datepicker-group').prev().find('table tr td').not('.datepick-other-month').not('.ui-datepicker-other-month').last();
                            tdPrev = tdStart;
                            if (tdPrev.length == 0) {
                    ...
                    while ((tdStart.hasClass('ui-datepicker-unselectable') || tdStart.hasClass('datepick-unselectable')) && !tdStart.hasClass('manualadded')) {
                        tdStart = tdStart.prev();
                    }
            ...
            if(inst.dates.length == 1 || flickerEnabled) {
                ...
                for (iCounter = 0; iCounter < counterTotal; iCounter++) {
                    ...
                    while (true) {
                        if (tdPrev.hasClass('ui-datepicker-other-month') || tdPrev.hasClass('datepick-other-month')) {
                            tdStart = tdStart.closest('.datepick-one-month, .ui-datepicker-group').next().find('table tr td').not('.datepick-other-month').not('.ui-datepicker-other-month').first();
                            tdPrev = tdStart;
                            if (tdPrev.length == 0) {
                                monthNotVisible = true;
                                break;
			...
			if ($jppr.datepick._get(inst, 'showStatus')) {
                ...
                if(useFlicker) {
                    ...
                    if ($jppr('#' + inst.id).find('.datePicker').parent().parent().attr('showLegend') == '1') {
                        ...
                        '<ul>' +
                        '<li><div class="legend-ico legend-start-s"></div><div>Start Date</div></li>' +

                        '<li><div class="legend-ico legend-unavailable-s"></div><div>Unavailable</div></li>' +
                        '</ul>' +
                        '</div>';
                    }
                }
