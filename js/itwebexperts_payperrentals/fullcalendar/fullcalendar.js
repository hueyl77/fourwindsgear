	...
	function _fetchEventSource(source, callback) {
		...
		if (events) {
			...
			if (url) {
                ...
                data['current_view'] = getView().name;
				pushLoading();
...
function ResourceView(element, calendar, viewName) {
	...
	function buildSkeleton(maxRowCnt, showNumbers) {
		...
		s =
			"<table class='fc-border-separate' style='width:100%' cellspacing='0'>" +
			"<thead>" +
			"<tr class='fc-first fc-last'><th class='fc-resourceName' style='width:230px;'>&nbsp;</th>";
		for (i=0; i<colCnt; i++) {
			s +=
				"<th class='fc- " + headerClass + "'/>"; // need fc- for setDayID
		}
		...
		for (i=0; i<maxRowCnt; i++) {
			...
			s +=
				"<tr class='fc-week" + id + "'><td class='fc-resourceName "+encodeURIComponent(resources[i]['id'])+"'>" + resourceName + "</td>";
			for (j=0; j<colCnt; j++) {
				s +=
