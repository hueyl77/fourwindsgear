var DymoPrinting = function (LabelPrinter) {
	...
	var PageOne = jQuery('<table width="100%" cellspacing="20">' +
		'<tbody>' +
		'<tr>' +
		(jQuery.inArray('8160-b', LabelPrinter.option('labelTypes')) > -1 ? '<td data-label_type="8160-b" align="center" class="ui-corner-all" style="border:2px solid white;"><img src="' + Image8160 + '"><br>Address<br>( For: Barcode )</td>' : '') +
		(jQuery.inArray('8160-s', LabelPrinter.option('labelTypes')) > -1 ? '<td data-label_type="8160-s" align="center" class="ui-corner-all" style="border:2px solid white;"><img src="' + Image8160 + '"><br>Address<br>( For: Shipping )</td>' : '') +
		(jQuery.inArray('8164', LabelPrinter.option('labelTypes')) > -1 ? '<td data-label_type="8164" align="center" class="ui-corner-all" style="border:2px solid white;"><img src="' + Image8164 + '"><br>Shipping<br>( For: Product Info )</td>' : '') +
		'</tr>' +
		'</tbody>' +
		'</table>');

	var PageTwo = jQuery('<table>' +
		'<tbody></tbody>' +
		'</table>');

	var PageThree = jQuery('<table>' +
		'<tbody></tbody>' +
		'</table>');

	var PageFour = jQuery('<div style=""></div>');
		...
		PageOne.find('td').each(function () {
			jQuery(this)
				...
				.click(function () {
					...
					var labelType = jQuery(this).data('label_type');
	...
	this.loadPageOne = function (isBack) {
		...
		if (Printers.length > 1){
			...
			jQuery.each(Printers, function () {
				var input = jQuery('<input type="radio" name="printerName" value="' + this.name + '" />')
			...
			LabelPrinter.setDialogButtons({
				...
				'Select Printer' : function () {
					SelectedPrinter = jQuery('input[name=printerName]:checked').data('printerInfo');
		...
		if (SelectedPrinter.isTwinTurbo){
			...
			PageThree.find('tbody .twinTurboReelSelect').each(function () {
				jQuery(this)
					...
					.mouseout(function () {
						if (!jQuery(this).hasClass('ui-state-selected')){
					...
					.click(function () {
						jQuery('.twinTurboReelSelect.ui-state-selected')
							.css('border-color', '#fff')
							.removeClass('ui-state-selected');

						jQuery(this).addClass('ui-state-selected');

						LabelPrinter.setUserData('selectedReel', jQuery(this).data('reel'));
					});
			});
		}
		...
		LabelPrinter.setDialogButtons({
			...
			'Preview Labels' : function () {
				PageThree.find('input[type=radio]:checked, select').each(function () {
					LabelPrinter.setUserData(jQuery(this).attr('name'), jQuery(this).val());
				});
	...
	this.loadPageFour = function (isBack) {
		...
		jQuery.ajax({
			...
			success  : function (resp) {
				...
				jQuery.each(resp.labelInfo.data, function () {
				...
				PageFour.empty();
				jQuery.each(previewImages, function (k, v) {
				...
				PageFour.find('.printStatusButton').click(function () {
					var image = jQuery(this).parent().find('img');
					if (image.hasClass('ui-state-disabled')){
						image.removeClass('ui-state-disabled');
						jQuery(this).parent().find('.zoomButton').button('enable');
						jQuery(this).find('span').html(TEXT_BUTTON_PRINT_NO);
					}
					else {
						image.addClass('ui-state-disabled');
						jQuery(this).parent().find('.zoomButton').button('disable');
						jQuery(this).find('span').html(TEXT_BUTTON_PRINT_YES);
					}
				});
				...
				PageFour.find('.zoomButton').click(function (e) {
					if (jQuery(this).hasClass('zoomed')){
						...
						jQuery(this).parent().find('.printStatusButton').button('enable');
						jQuery(this).find('span').html(TEXT_BUTTON_ZOOM);
						jQuery(this).data('clonedImg').animate({
							width  : jQuery(this).data('original_width'),
							height : jQuery(this).data('original_height'),
							top    : jQuery(this).data('original_top'),
							left   : jQuery(this).data('original_left')
						}, {
							duration : 1000,
							complete : function () {
								jQuery(this).remove();
						...
						var image = jQuery(this).parent().find('img');
						jQuery(this).parent().find('.printStatusButton').button('disable');
						jQuery(this).addClass('zoomed');
						jQuery(this).find('span').html(TEXT_BUTTON_UNZOOM);
						...
						jQuery(this).data('clonedImg', cloneImg);
						jQuery(this).data('original_width', image.width());
						jQuery(this).data('original_height', image.height());
						jQuery(this).data('original_top', image.offset().top);
						jQuery(this).data('original_left', image.offset().left);
					}
					return false;
				});
		...
		PageFour.find('.printStatusButton').each(function () {
			...
			if (image.hasClass('ui-state-disabled') === false){
				dymo.label.framework.printLabel(
					...
					printParams,
					labels[jQuery(this).data('preview_index')].labelXml
				);
			}
		});
	};
