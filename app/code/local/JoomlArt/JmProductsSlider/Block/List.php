...
class JoomlArt_JmProductsSlider_Block_List extends Mage_Catalog_Block_Product_Abstract {
    ...
    const CACHE_TAG = 'jm_productsslider';
    ...
    public function __construct($attributes = array()) {
        ...
        //get cache status
        $cacheType = 'joomlart_jmproductsslider';
        $useCache = Mage::app()->useCache($cacheType);
        $cacheId = $this->getCacheId($attributes);
        if ($useCache){
            $this->addData(array(
                'cache_key' => $cacheId,
    ...
    function getListTopRatedProducts($orderfeild = 'rating_summary', $order = 'desc', $perPage = NULL, $currentPage = 1) {
        ...
        $products->getSelect()->joinLeft(
            array('_reviewed_order_table' => Mage::getConfig()->getTablePrefix() . 'review_entity_summary'), "_reviewed_order_table.store_id={$storeId} AND _reviewed_order_table.entity_pk_value=e.entity_id", array()
        );
    ...
    function set(
    $show = 1, $mode = '', $title = '', $height = '', $width = '', $itemheight = '', $itemwidth = '', $catsid = '', $quanlity = '', $number_items = '', $show_price = "", $show_cart = '', $use_scrollerbar = '', $autorun = '', $delaytime = '', $animationtime = '', $attributename = '', $attributevalue = '', $template = '', $max = '80') {
        if (!$show) {
            $this->_config['show'] = 0;
            return;
        }
        ...
        if ($quanlity)
            $this->_config['qty'] = $quanlity;
        if ($number_items)
            $this->_config['number_items'] = $number_items;
        if ($show_price)
            $this->_config['show_price'] = $show_price;
        if ($show_cart)
            $this->_config['show_cart'] = $show_cart;
        if ($use_scrollerbar)
            $this->_config['use_scrollerbar'] = $use_scrollerbar;
        if ($autorun)
            $this->_config['autorun'] = $autorun;
        if ($delaytime)
            $this->_config['delaytime'] = $delaytime;
        if ($animationtime)
            $this->_config['animationtime'] = $animationtime;
        if ($attributename)
            $this->_config['attributename'] = $attributename;
        if ($attributevalue)
            $this->_config['attributevalue'] = $attributevalue;
        if ($template)
    ...
    function jmResize($image, $max_width, $max_height, $crop = 1) {
        ...
        if (!file_exists($path . '/media/resized/') && !mkdir($path . '/media/resized/', 0755))
            return '';
        ...
        switch ($imgInfo[2]) {
            case 1: $im = imagecreatefromgif($full_path);
                break;
            case 2: $im = imagecreatefromjpeg($full_path);
                break;
            case 3: $im = imagecreatefrompng($full_path);
                break;
            default: return '';
                break;
        }
